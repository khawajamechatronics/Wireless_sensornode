...
void signerAtsha204SoftInit(void)
{
	...
	// Set secrets
	MyHwPsoc_ReadConfigBlock((void*)_signing_hmac_key, EEPROM_SIGNING_SOFT_HMAC_KEY_ADDRESS, 32);
	MyHwPsoc_ReadConfigBlock((void*)_signing_node_serial_info, EEPROM_SIGNING_SOFT_SERIAL_ADDRESS, 9);
}
...
bool signerAtsha204SoftGetNonce(MyMessage *msg)
{
    ...
    int i;
	for (i = 0; i < 32; i++) {
		sha256_write(MyHwPsoc_Random(0,256) ^ (TIMER1_ReadCounter()&0xFF));
	}
