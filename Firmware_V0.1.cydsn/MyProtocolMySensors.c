...
bool protocolParse(MyMessage *message, char *inputString)
{
	...
	if (command == C_STREAM) {
        MyMessage_set(message, bvalue, P_CUSTOM, blen, 0);
	} else {
        MyMessage_set(message, value, P_BYTE, 0, 0);
	}
	return true;
}
...
char * protocolFormat(MyMessage *message)
{
	snprintf_P( _fmtBuffer, 
                ...
                message->sender,
	            message->sensor, 
                (uint8_t)mGetCommand(message), 
                (uint8_t)mGetAck(message), 
                message->type,
	            MyMessage_getString(message, _convBuffer));
	return _fmtBuffer;
}
