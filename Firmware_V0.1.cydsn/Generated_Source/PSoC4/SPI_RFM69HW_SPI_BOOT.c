...
/*******************************************************************************
* Function Name: SPI_RFM69HW_SpiCyBtldrCommStart
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: SPI_RFM69HW_SpiCyBtldrCommStop
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: SPI_RFM69HW_SpiCyBtldrCommReset
****************************************************************************//**
*
...
void SPI_RFM69HW_SpiCyBtldrCommReset(void)
{
    ...
    SPI_RFM69HW_SpiUartClearRxBuffer();
    SPI_RFM69HW_SpiUartClearTxBuffer();
}


/*******************************************************************************
* Function Name: SPI_RFM69HW_SpiCyBtldrCommRead
****************************************************************************//**
*
...
cystatus SPI_RFM69HW_SpiCyBtldrCommRead(uint8 pData[], uint16 size, uint16 * count, uint8 timeOut)
{
    ...
    if ((NULL != pData) && (size > 0u))
    {
        ...
        {
            /* Check packet start */
            if (0u != SPI_RFM69HW_SpiUartGetRxBufferSize())
            {
                /* Wait for packet end */
                do
                {
                    byteCount = SPI_RFM69HW_SpiUartGetRxBufferSize();
                    CyDelayUs(SPI_RFM69HW_SPI_BYTE_TO_BYTE);
                }
                while (byteCount != SPI_RFM69HW_SpiUartGetRxBufferSize());

                /* Disable data reception into RX FIFO */
                SPI_RFM69HW_RX_FIFO_CTRL_REG |= SPI_RFM69HW_RX_FIFO_CTRL_FREEZE;

                byteCount = SPI_RFM69HW_BYTES_TO_COPY(byteCount, size);
        ...
        {
            pData[i] = (uint8) SPI_RFM69HW_SpiUartReadRxData();
        }
    }
...
/*******************************************************************************
* Function Name: SPI_RFM69HW_SpiCyBtldrCommWrite
****************************************************************************//**
*
*  Allows the caller to read data from the bootloader host (the host writes the
*  data). The function handles polling to allow a block of data to be completely
...
cystatus SPI_RFM69HW_SpiCyBtldrCommWrite(const uint8 pData[], uint16 size, uint16 * count, uint8 timeOut)
{
    ...
    if ((NULL != pData) && (size > 0u))
    {
        ...
        SPI_RFM69HW_SpiUartPutArray(pData, (uint32) size);

        /* Wait with timeout 1mS for packet end */
        do
        {
            /* Check for packet end */
            if ((0u == SPI_RFM69HW_SpiUartGetTxBufferSize()) &&
                (0u == (SPI_RFM69HW_SPI_STATUS_REG & SPI_RFM69HW_SPI_STATUS_BUS_BUSY)) &&
                (0u == (SPI_RFM69HW_TX_FIFO_STATUS_REG & SPI_RFM69HW_TX_FIFO_SR_VALID)))
            {
                *count = size;
                status = CYRET_SUCCESS;
        ...
        SPI_RFM69HW_SpiUartClearRxBuffer();
        SPI_RFM69HW_RX_FIFO_CTRL_REG &= (uint32) ~SPI_RFM69HW_RX_FIFO_CTRL_FREEZE;
    }
