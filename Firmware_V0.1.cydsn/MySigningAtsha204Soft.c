...
void signerAtsha204SoftInit(void)
{
	...
	MyHwPsoc_RandomNumberInit();
	// Set secrets
	MyHwPsoc_ReadConfigBlock((void*)_signing_hmac_key, (void*)EEPROM_SIGNING_SOFT_HMAC_KEY_ADDRESS, 32);
	MyHwPsoc_ReadConfigBlock((void*)_signing_node_serial_info, (void*)EEPROM_SIGNING_SOFT_SERIAL_ADDRESS, 9);
}

bool signerAtsha204SoftCheckTimer(void)
{
	if (_signing_verification_ongoing) {
		if (TIMER1_ReadCounter() < _signing_timestamp ||
		        TIMER1_ReadCounter() > _signing_timestamp + MY_VERIFICATION_TIMEOUT_MS) {
...
bool signerAtsha204SoftGetNonce(MyMessage *msg)
{
    ...
    int i;
	for (i = 0; i < 32; i++) {
		sha256_write(MyHwPsoc_Random(0,256) ^ (MyHwPsoc_hwMillis()&0xFF));
	}
	memcpy(_signing_verifying_nonce, sha256_result(), MAX_PAYLOAD);
    ...
    MyMessage_set( msg, &_signing_verifying_nonce, P_CUSTOM , MAX_PAYLOAD, 0);
	_signing_verification_ongoing = true;
	_signing_timestamp = TIMER1_ReadCounter(); // Set timestamp to determine when to purge nonce
	...
	if (_signing_timestamp + MY_VERIFICATION_TIMEOUT_MS < TIMER1_ReadCounter()) {
...
void signerAtsha204SoftPutNonce(MyMessage *msg)
{
	...
	memcpy(_signing_signing_nonce, (uint8_t*)MyMessage_getCustom(msg), MAX_PAYLOAD);
	// We set the part of the 32-byte nonce that does not fit into a message to 0xAA
...
bool signerAtsha204SoftSignMsg(MyMessage *msg)
{
	...
	if (DO_WHITELIST(msg->destination)) {
        ...
        int i;
		for (i=0; i<32; i++) {
			sha256_write(_signing_hmac[i]);
		}
		sha256_write(msg->sender);
		for (i=0; i<SHA204_SERIAL_SZ; i++) {
			sha256_write(_signing_node_serial_info[i]);
		}
		memcpy(_signing_hmac, sha256_result(), 32);
    ...
    memcpy(&(msg->payload_data.data[mGetLength(msg)]), _signing_hmac, MAX_PAYLOAD-mGetLength(msg));
	DEBUG_SIGNING_PRINTBUF(F("Signature in message: "), (uint8_t*)*msg->payload_data.data[mGetLength(msg)],
...
bool signerAtsha204SoftVerifyMsg(MyMessage *msg)
{
	if (!_signing_verification_ongoing) {
		...
		if (msg->payload_data.data[mGetLength(msg)] != SIGNING_IDENTIFIER) {
		...
		for (j=0; j < NUM_OF(_signing_whitelist); j++) {
			if (_signing_whitelist[j].nodeId == msg->sender) {
				...
				for (int i=0; i<SHA204_SERIAL_SZ; i++) {
					sha256_write(_signing_whitelist[j].serial[i]);
				}
				memcpy(_signing_hmac, sha256_result(), 32);
				DEBUG_SIGNING_PRINTBUF(F("SHA256: "), _signing_hmac, 32);
				break;
		...
		if (signerMemcmp(msg->payload_data.data[mGetLength(msg)], _signing_hmac, MAX_PAYLOAD-mGetLength(msg))) {
...
static void signerCalculateSignature(MyMessage *msg, bool signing)
{
	...
	memcpy(_signing_temp_message, (uint8_t*)msg->payload_data.data[1-HEADER_SIZE],
	       MAX_MESSAGE_LENGTH-1-(MAX_PAYLOAD-mGetLength(msg)));
	DEBUG_SIGNING_PRINTBUF(F("Message to process: "), (uint8_t*)*msg->payload_data.data[1-HEADER_SIZE],
	                       MAX_MESSAGE_LENGTH-1-(MAX_PAYLOAD-mGetLength(msg)));
    ...
    int i;
	for (i=0; i<32; i++) {
		sha256_write(_signing_temp_message[i]);
	}
	sha256_write(0x15); // OPCODE
	sha256_write(0x02); // param1
	sha256_write(0x08); // param2(1)
	sha256_write(0x00); // param2(2)
	sha256_write(0xEE); // SN[8]
	sha256_write(0x01); // SN[0]
	sha256_write(0x23); // SN[1]
	for (i=0; i<25; i++) {
		sha256_write(0x00);
	}
	for (i=0; i<32; i++) {
		sha256_write(signing ? _signing_signing_nonce[i] : _signing_verifying_nonce[i]);
	}
	// Purge nonce when used
	memset(signing ? _signing_signing_nonce : _signing_verifying_nonce, 0xAA, 32);
	memcpy(_signing_temp_message, sha256_result(), 32);

	// Feed "message" to HMAC calculator
	sha256_initHmac(_signing_hmac_key,32); // Set the key to use
	for (i=0; i<32; i++) {
		sha256_write(0x00); // 32 bytes zeroes
	}
	for (i=0; i<32; i++) {
		sha256_write(_signing_temp_message[i]); // 32 bytes digest
	}
	sha256_write(0x11); // OPCODE
	sha256_write(0x04); // Mode
	sha256_write(0x00); // SlotID(1)
	sha256_write(0x00); // SlotID(2)
	for (i=0; i<11; i++) {
		sha256_write(0x00); // 11 bytes zeroes
	}
	sha256_write(0xEE); // SN[8]
	for (i=0; i<4; i++) {
		sha256_write(0x00); // 4 bytes zeroes
	}
	sha256_write(0x01); // SN[0]
	sha256_write(0x23); // SN[1]
	for (i=0; i<2; i++) {
		sha256_write(0x00); // 2 bytes zeroes
	}

	memcpy(_signing_hmac, sha256_resultHmac(), 32);
