...
// macros, saves space
#define RFM69_readReg(__reg) MyHwPsoc_RFM69_RAW_readByteRegister(__reg & RFM69_READ_REGISTER)
#define RFM69_writeReg(__reg, __value) MyHwPsoc_RFM69_RAW_writeByteRegister((__reg | RFM69_WRITE_REGISTER), __value )
#define RFM69_burstReadReg(__reg, __buf, __len) MyHwPsoc_RFM69_spiMultiByteTransfer( __reg & RFM69_READ_REGISTER, (uint8_t*)__buf, __len, true )
#define RFM69_burstWriteReg(__reg, __buf, __len) MyHwPsoc_RFM69_spiMultiByteTransfer( __reg | RFM69_WRITE_REGISTER, (uint8_t*)__buf, __len, false )
...
LOCAL void RFM69_interruptHandler(void)
{
	...
	if ( RFM69.radioMode == RFM69_RADIO_MODE_RX && (regIrqFlags2 & RFM69_IRQFLAGS2_PAYLOADREADY) ) {
		RFM69.currentPacket.RSSI = RFM69_readRSSI(false);
		(void)RFM69_setRadioMode(RFM69_RADIO_MODE_STDBY);
...
LOCAL uint8_t RFM69_recv(uint8_t* buf, const uint8_t maxBufSize)
{
	...
	if ((controlFlags & RFM69_ACK_REQUESTED) && !(controlFlags & RFM69_ACK_RECEIVED)) {
...
LOCAL bool RFM69_sendFrame(rfm69_packet_t* packet, const bool increaseSequenceCounter)
{
	...
	*/
	...
	const uint32_t txStart = MyHwPsoc_hwMillis();
	// irq handler set radiomode to rx once packet sent
	while (!RFM69.dataSent && MyHwPsoc_hwMillis() - txStart < RFM69_TX_LIMIT_MS) { // wait until packet sent
		doYield();
	}
                         ...
                         const rfm69_RSSI_t RSSI)
{
    ...
    flags |= RFM69_ACK_RECEIVED;
    flags |= RFM69_ACK_RSSI_REPORT;
	(void)RFM69_send(recipient, (uint8_t*)&ACK, sizeof(rfm69_ack_t), flags, true);
}
                               ...
                               const uint8_t bufferSize, const uint8_t retries, const uint32_t retryWaitTimeMS)
{
	for (uint8_t retry = 0; retry <= retries; retry++) {
		...
		while (MyHwPsoc_hwMillis() - enterMS < retryWaitTimeMS) {
			if (RFM69.ackReceived) {
				...
				if (sender == recipient && ACKsequenceNumber == RFM69.txSequenceNumber) {
					...
					// ATC
					if (RFM69.ATCenabled && (flags & RFM69_ACK_RSSI_REPORT)) {
		...
		// random 100ms
		const uint32_t enterCSMAMS = MyHwPsoc_hwMillis();
		const uint16_t randDelayCSMA = enterMS & 40; // 61 bytes + ACK take ~40ms
		while (MyHwPsoc_hwMillis() - enterCSMAMS < randDelayCSMA) {
			doYield();
		}
	}
	return false;
...
LOCAL int16_t RFM69_getSendingRSSI(void)
{
	...
	if ((RFM69.currentPacket.header.controlFlags & RFM69_ACK_RSSI_REPORT)) {
...
LOCAL bool RFM69_setTxPowerPercent(uint8_t newPowerPercent)
{
	...
	const rfm69_powerlevel_t newPowerLevel = (rfm69_powerlevel_t)(RFM69_MIN_POWER_LEVEL_DBM + (RFM69_MAX_POWER_LEVEL_DBM
...
{
	// report TX level in %
	const uint8_t result = (uint8_t)(100.0f * (RFM69.powerLevel -
