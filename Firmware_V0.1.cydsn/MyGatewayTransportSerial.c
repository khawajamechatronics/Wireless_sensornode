...
bool gatewayTransportSend(MyMessage *message)
{
    ...
    MyHwPsoc_SerialPrint(protocolFormat(message));
...
{
    buildGw(&_msgTmp, I_GATEWAY_READY);
    MyMessage_set(&_msgTmp, MSG_GW_STARTUP_COMPLETE, P_STRING, 0, 0);
	(void)gatewayTransportSend(&_msgTmp);
	// Send presentation of  ly attached sensors (and node if applicable)
	presentNode();
	return true;
}
...
{
	while (MyHwPsoc_SerialAvailable()) {
		// get the new byte:
		const char inChar = (char)MyHwPsoc_SerialRead();
