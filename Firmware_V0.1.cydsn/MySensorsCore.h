...
*/
bool present(const uint8_t sensorId, const uint8_t sensorType, const char *description,     //cb changed description = "", ack = FALSE;
             const bool ack);  
 ...
 * @return TRUE Returns TRUE if message reached the first stop on its way to destination.
 */
bool sendSketchInfo(const char *name, const char *version, const bool ack);  //CB changed ack = FALSE
...
* @return TRUE Returns TRUE if message reached the first stop on its way to destination.
*/
bool send(MyMessage *msg, const bool ack);  //CB ack = FALSE
 ...
 * @return TRUE Returns TRUE if message reached the first stop on its way to destination.
 */
bool sendBatteryLevel(const uint8_t level, const bool ack); //CB ack = FALSE
 ...
 * @return TRUE Returns TRUE if message reached the first stop on its way to destination.
 */
bool sendHeartbeat(const bool ack); //CB ack = FALSE
...
bool request(const uint8_t childSensorId, const uint8_t variableType,
             const uint8_t destination);              //CB destination = GATEWAY_ADDRESS     
 ...
 * @return TRUE Returns TRUE if message reached the first stop on its way to destination.
 */
bool requestTime(const bool ack);   //CB ack = FALSE
...
/**
 * Save a state (in   EEPROM). Good for actuators to "remember" state between
 * power cycles.
 *
 ...
 */
int8_t sleep(const uint32_t sleepingMS, const bool smartSleep); //CB smartSleep = FALSE
 ...
 */
int8_t sleep1(const uint8_t interrupt, const uint8_t mode, const uint32_t sleepingMS,    //CB sleepingMS = 0
             const bool smartSleep);                                                    //CB smartSleep = FALSE
...
int8_t sleep2(const uint8_t interrupt1, const uint8_t mode1, const uint8_t interrupt2,
             const uint8_t mode2, const uint32_t sleepingMS, const bool smartSleep);  //CB sleepingMS = 0 , smartSleep = FALSE
...
* @return Interrupt number if wake up was triggered by pin change, @ref MY_WAKE_UP_BY_TIMER if wake up was triggered by timer, @ref MY_SLEEP_NOT_POSSIBLE if sleep was not possible (e.g. ongoing FW update)
*/
int8_t smartSleep1(const uint8_t interrupt, const uint8_t mode, const uint32_t sleepingMS);         //CB sleepingMS = 0
...
int8_t smartSleep2(const uint8_t interrupt1, const uint8_t mode1, const uint8_t interrupt2,
                  const uint8_t mode2, const uint32_t sleepingMS);                              //CB sleepingMS = 0
...
* @return Interrupt number if wake up was triggered by pin change, @ref MY_WAKE_UP_BY_TIMER if wake up was triggered by timer, @ref MY_SLEEP_NOT_POSSIBLE if sleep was not possible (e.g. ongoing FW update)
*/
int8_t _sleep(const uint32_t sleepingMS, const bool smartSleep,  //CB smartSleep = FALSE
              const uint8_t interrupt1, const uint8_t mode1,     //CB interrupt1 = INTERRUPT_NOT_DEFINED , mode1 = MODE_NOT_DEFINED
              const uint8_t interrupt2, const uint8_t mode2);    //CB interrupt2 = INTERRUPT_NOT_DEFINED mode2 = MODE_NOT_DEFINED
...
* @return TRUE Returns TRUE if message reached the first stop on its way to destination.
*/
bool _sendRoute(MyMessage *message);        //CB & to *

/**
* @brief Callback for incoming messages
* @param message
*/
void receive(const MyMessage *message)  __attribute__((weak));  //CB & to *
/**
...
static inline MyMessage* build(MyMessage *msg, const uint8_t destination, const uint8_t sensor,
                               const uint8_t command, const uint8_t type, const bool ack)  //CB ack = FALSE
{
