...
void _begin(void)
{
	...
	MyHwPsoc_ReadConfigBlock(   (void*)&_coreConfig.controllerConfig, 
                                (void*)EEPROM_CONTROLLER_CONFIG_ADDRESS,
	...
	MyHwPsoc_WriteConfig(EEPROM_PARENT_NODE_ID_ADDRESS, MY_PARENT_NODE_ID);
...
void _registerNode(void)
{
	...
	do {
        build(&_msgTmp, GATEWAY_ADDRESS, NODE_SENSOR_ID, C_INTERNAL,I_REGISTRATION_REQUEST,false);
        uint8_t temp_data = MY_CORE_VERSION;
        MyMessage_set(&_msgTmp, &temp_data, P_BYTE, 0, 0);
		(void)_sendRoute(&_msgTmp);
	} while (!wait_cmd(2000, C_INTERNAL, I_REGISTRATION_RESPONSE) && counter--);
#else
...
void presentNode(void)
{
    ...
    build(&_msgTmp, GATEWAY_ADDRESS, NODE_SENSOR_ID, C_INTERNAL,I_CONFIG,false);
    uint8_t tempdata = getParentNodeId();
    MyMessage_set(&_msgTmp, &tempdata, P_BYTE, 0, 0);
	(void)_sendRoute(&_msgTmp);

	// Wait configuration reply.
	(void)wait_cmd(2000, C_INTERNAL, I_CONFIG);
...
{
	build(&_msgTmp, GATEWAY_ADDRESS, NODE_SENSOR_ID, C_INTERNAL, I_BATTERY_LEVEL,ack);
    MyMessage_set(&_msgTmp, (void*)&value, P_BYTE, 0, 0);
    return _sendRoute(&_msgTmp);
}
...
bool sendHeartbeat(const bool ack)
{
    ...
    build(&_msgTmp, GATEWAY_ADDRESS, NODE_SENSOR_ID, C_INTERNAL, I_HEARTBEAT_RESPONSE,ack);
    MyMessage_set(&_msgTmp, &heartbeat, P_ULONG32, 0, 0); 
    return _sendRoute(&_msgTmp);
#else
	(void)ack;
	return false;
...
bool present(const uint8_t childSensorId, const uint8_t sensorType, const char *description,
             const bool ack)
{
    build(&_msgTmp, GATEWAY_ADDRESS, childSensorId, C_PRESENTATION, sensorType,ack);
    description = (childSensorId==NODE_SENSOR_ID?MYSENSORS_LIBRARY_VERSION:description);
    MyMessage_set(&_msgTmp, (void*)description, P_STRING, 0, 0);
    return _sendRoute(&_msgTmp);
}
...
{
	bool result = true;
    build(&_msgTmp, GATEWAY_ADDRESS, NODE_SENSOR_ID, C_INTERNAL, I_SKETCH_NAME, ack);
	if (name) { 
        MyMessage_set(&_msgTmp, (void*)name, P_STRING, 0, 0);
        result &= _sendRoute(&_msgTmp);
	}
    build(&_msgTmp, GATEWAY_ADDRESS, NODE_SENSOR_ID, C_INTERNAL, I_SKETCH_VERSION, ack);
	if (version) {
        MyMessage_set(&_msgTmp, (void*)version, P_STRING, 0, 0);
        result &= _sendRoute(&_msgTmp);
	}
...
{
    build(&_msgTmp, destination, childSensorId, C_REQ, variableType, false);
...
{
    build(&_msgTmp, GATEWAY_ADDRESS, NODE_SENSOR_ID, C_INTERNAL, I_TIME, ack);
    MyMessage_set(&_msgTmp, "", P_STRING, 0, 0);
	return _sendRoute(&_msgTmp);
}
...
bool _processInternalMessages(void)
{
	...
	if (_msg.sender == GATEWAY_ADDRESS) {
		if (type == I_REBOOT) {
			...
			_coreConfig.controllerConfig.isMetric = _msg.payload_data.data[0] == 0x00 ||
			                                        _msg.payload_data.data[0] == 'M'; // metric if null terminated or M
			MyHwPsoc_WriteConfigBlock((void*)&_coreConfig.controllerConfig, EEPROM_CONTROLLER_CONFIG_ADDRESS, 
			                   sizeof(controllerConfig_t));
...
#if defined(MY_REPEATER_FEATURE)
			if (_msg.payload_data.data[0] == 'C') {
				// Clears child relay data for this node
				setIndication(INDICATION_CLEAR_ROUTING);
				transportClearRoutingTable();
                build(&_msgTmp, GATEWAY_ADDRESS, NODE_SENSOR_ID, C_INTERNAL, I_CHILDREN,false);
                MyMessage_set(&_msgTmp, "OK", P_STRING, 0, 0);
				(void)_sendRoute(&_msgTmp);
			}
#endif
		} else if (type == I_DEBUG) {
#if defined(MY_DEBUG) || defined(MY_SPECIAL_DEBUG)
			const char debug_msg = _msg.payload_data.data[0];
			if (debug_msg == 'R') {		// routing table
#if defined(MY_REPEATER_FEATURE)
                uint16_t cnt = 0;
				for (cnt = 0; cnt < SIZE_ROUTES; cnt++) {
					...
					if (route != BROADCAST_ADDRESS) {
                        ...
                        MyMessage_set(&_msgTmp, outBuf, P_CUSTOM, 2, 0);
						(void)_sendRoute(&_msgTmp);
						wait(200);
					}
				}
                ...
                build(&_msgTmp, GATEWAY_ADDRESS, NODE_SENSOR_ID, C_INTERNAL, I_DEBUG, false);
                uint16_t tmp = hwCPUVoltage();
                MyMessage_set(&_msgTmp, &tmp, P_UINT16, 0, 0);
                (void)_sendRoute(&_msgTmp);
			} else if (debug_msg == 'F') {	// CPU frequency in 1/10Mhz
                build(&_msgTmp, GATEWAY_ADDRESS, NODE_SENSOR_ID, C_INTERNAL, I_DEBUG, false);
                uint16_t tmp = hwCPUFrequency();
                MyMessage_set(&_msgTmp, &tmp, P_UINT16, 0, 0);
                (void)_sendRoute(&_msgTmp);
			} else if (debug_msg == 'M') {	// free memory
                build(&_msgTmp, GATEWAY_ADDRESS, NODE_SENSOR_ID, C_INTERNAL, I_DEBUG, false);
				uint16_t tmp = hwFreeMem();
                MyMessage_set(&_msgTmp, &tmp, P_UINT16, 0, 0);
                (void)_sendRoute(&_msgTmp);
				...
				(void)_sendRoute(&_msgTmp);
                int i;
				for (i = EEPROM_START; i<EEPROM_LOCAL_CONFIG_ADDRESS; i++) {
					MyHwPsoc_WriteConfig(i, 0xFF);
				}
				setIndication(INDICATION_REBOOT);
				MyHwPsoc_Reboot();
			}
		...
		if (type == I_REGISTRATION_REQUEST) {
			...
			delay(5);
#endif
            build(&_msgTmp, _msg.sender, NODE_SENSOR_ID, C_INTERNAL, I_REGISTRATION_RESPONSE, false);
            MyMessage_set(&_msgTmp, &approveRegistration, P_BYTE, 0, 0);
			(void)_sendRoute(&_msgTmp);
#else
...
{
	MyHwPsoc_WriteConfig(EEPROM_LOCAL_CONFIG_ADDRESS+pos, value);
}
uint8_t loadState(const uint8_t pos)
{
	return MyHwPsoc_ReadConfig(EEPROM_LOCAL_CONFIG_ADDRESS+pos);
}


void wait(const uint32_t waitingMS)
{
	const uint32_t enteringMS = TIMER1_ReadCounter();
	while (TIMER1_ReadCounter() - enteringMS < waitingMS) {
		_process();
	}
}

bool wait_cmd(const uint32_t waitingMS, const uint8_t cmd, const uint8_t msgType)
{
	const uint32_t enteringMS = TIMER1_ReadCounter();
	// invalidate msg type
	_msg.type = !msgType;
	bool expectedResponse = false;
	while ( (TIMER1_ReadCounter() - enteringMS < waitingMS) && !expectedResponse ) {
		_process();
		expectedResponse = (BF_GET(_msg.command_ack_payload, 0, 3) == cmd && _msg.type == msgType);
	}
              ...
              const uint8_t mode1, const uint8_t interrupt2, const uint8_t mode2)
{
	...
	if (interrupt1 != INTERRUPT_NOT_DEFINED && interrupt2 != INTERRUPT_NOT_DEFINED) {
		// both IRQs
		result = hwSleep2(interrupt1, mode1, interrupt2, mode2, sleepingTimeMS);
	} else if (interrupt1 != INTERRUPT_NOT_DEFINED && interrupt2 == INTERRUPT_NOT_DEFINED) {
		// one IRQ
		result = hwSleep1(interrupt1, mode1, sleepingTimeMS);
	} else if (interrupt1 == INTERRUPT_NOT_DEFINED && interrupt2 == INTERRUPT_NOT_DEFINED) {
		// no IRQ
...
int8_t sleep1(const uint8_t interrupt, const uint8_t mode, const uint32_t sleepingMS,
             const bool smartSleep)
{
	return _sleep(sleepingMS, smartSleep, interrupt, mode, INTERRUPT_NOT_DEFINED, MODE_NOT_DEFINED);
}

int8_t sleep2(const uint8_t interrupt1, const uint8_t mode1, const uint8_t interrupt2,
...
int8_t smartSleep1(const uint8_t interrupt, const uint8_t mode, const uint32_t sleepingMS)
{
	// compatibility
	return _sleep(sleepingMS, true, interrupt, mode, INTERRUPT_NOT_DEFINED, MODE_NOT_DEFINED);
}

int8_t smartSleep2(const uint8_t interrupt1, const uint8_t mode1, const uint8_t interrupt2,
