...
*/
LOCAL bool RFM69_sendFrame(rfm69_packet_t* packet, const bool increaseSequenceCounter);  //const bool increaseSequenceCounter = true
...
LOCAL bool RFM69_send(const uint8_t recipient, uint8_t* data, const uint8_t len,
                      const rfm69_controlFlags_t flags, const bool increaseSequenceCounter);  //const bool increaseSequenceCounter = true
...
LOCAL bool RFM69_sendWithRetry(const uint8_t recipient, const void* buffer,
                               ...
                               const uint8_t retries, const uint32_t retryWaitTimeMS);
                                       // const uint8_t retries = RFM69_RETRIES, const uint32_t retryWaitTimeMS
...
*/
LOCAL rfm69_RSSI_t RFM69_readRSSI(bool forceTrigger);  //forceTrigger = false
...
*/
LOCAL void RFM69_ATCmode(const bool onOff, const int16_t targetRSSI);  //const int16_t targetRSSI = RFM69_TARGET_RSSI_DBM
